...
In a previous [article](https://jurajburian.github.io/posts/cats-user-demo/), 
I demonstrated a simple application using Cats and Cats Effect. We used an approach where errors were hidden from the API. 
This approach can work well for small services with simple APIs.
...
## Motivation
In general, we want to create precise descriptions of result types, including error types, at the API level.
...
In Scala 3, we can use type operators like `&` (intersection) and `|` (union) to define types. For example, we can specify that a result will be of type `Int | String`. This powerful abstraction allows us to define precise descriptions of result types and function parameters. But can we use this mechanism effectively for describing errors at the API level?

> An open error model is a practical solution to certain limitations in Scala's compile-time type resolution. Specifically, it addresses the fact that Scala's compiler resolves to the most abstract type by default. When using sealed trait hierarchies for errors, this prevents the use of union types for more precise error handling. 
...
case class InternalServerError(cause: String,
   essage: String = "Internal server error"
)
```
Let's examine how we can define our services:
```scala
...
The `E` type parameter in our service methods serves a crucial role in enabling flexible error composition. Let's break down why it's necessary and how it works.

When we chain operations in a for-comprehension, the compiler needs to unify the error types of all operations. The `E` type parameter allows us to:

1. **Preserve Error Information**: It maintains the ability to add more specific error types when methods are composed.
2. **Enable Flexible Composition**: It allows the same method to be used in different contexts where different error types might be relevant.
3. **Prevent Type Mismatches**: It helps the compiler verify that error types are properly propagated through the call chain.
...
If we look on signature of `EitherT:flatMap` method:
`def flatMap[AA >: A, D](f: B => EitherT[F, AA, D])(implicit F: Monad[F]): EitherT[F, AA, D]`
we can see that there is AA>:A - thus error type can only grow (become more general) through `flatMap` chains
...
#### When to Use [E] Type Parameter:<br>
...
## Catching error
A perfect example is the `UserRepository` service implementation:
```scala
class DoobieUserRepository[F[_]: Async](xa: Transactor[F]) extends UserRepository[F] {
...
```
We can see that we catch errors from the database and convert them to `InternalServerError` (error handling is simplified for demonstration purposes) in the `attemptDb` extension method. 
...
* The compiler cannot detect unused error types declared at the API level. However, it will catch cases where an error type is used but not declared in the method signature. 
* Using any parent in error model cause that childrens can't be distinguished in union type. For example: let say we have Error parent and two childrens: `Child1` and `Child2`. Then we have Error2 outside hierarchy. `Child1 | Child2 | Error2` is the same as `Error | Error2`!
* Errors transformations can be done by `leftMap` method.
